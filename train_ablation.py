...
def get_args():
    ...
    parser.add_argument("--name", default= 'ablation_' + time.strftime("[%Y-%m-%d__%H-%M-%S]", time.localtime()), help='Name of the experiment')
    parser.add_argument("--store_file", dest="store_file", default='results_ablation_' + time.strftime("[%Y-%m-%d__%H-%M-%S]", time.localtime()), help='file for storing the last results')
...
class Runner(object):
    ...
    def __init__(self, params):
        ...
        """
        self.p = params
        self.logger = get_logger('{}_{}.log'.format(self.p.dataset, self.p.name), self.p.log_dir, self.p.config_dir)
    ...
    def load_data(self):
        ...
        self.rel2id = json.load(open('./data/{}/rel2id_abl.json'.format(self.p.dataset),'r'))
    ...
    def store_results(self, results):
        ...
        results_table = PrettyTable([self.p.dataset, 'left/tail'])
        mr = ['MR', results['mr']]
        mrr = ['MRR', results['mrr']]
        hit = []
        for i in range(10):
            hit.append(['Hit@{}'.format(i + 1), results['hits@{}'.format(i + 1)]])
    ...
    def evaluate(self, split, epoch):
        ...
        right_results = [0] # predict head according to (tail rel_inv)
        
        results = left_results

        count = float(left_results['count'])

        results['mr'] = round(left_results['mr'] / count, 5)  # 保留五位小数 得到最终的mr结果
        results['mrr'] = round(left_results['mrr'] / count, 5)
        for k in range(10):
            results['hits@{}'.format(k + 1)] = round((left_results['hits@{}'.format(k + 1)] ) / (count), 5)  # 左右结合 hit


        self.logger.info('[Epoch {} {}]: Current_MRR: Tail : {:.5}'.format(epoch, split, results['mrr']))  # .5代表保留五位有效数字
        return results
    ...
    def fit(self):
        ...
        save_path = os.path.join('./torch_saved/', '{}_{}'.format(self.p.dataset, self.p.name))
        read_path = os.path.join('./torch_saved/', 'kinship_ablation_[2024-07-08__23-24-09]')
        if self.p.restore:
        ...
        for epoch in range(self.p.max_epochs):
            ...
            if val_results['mrr'] > self.best_val_mrr:
                ...
                self.logger.info('Better result and save model!')
                kill_cnt = 0
                self.p.margin = 10
            else:
                kill_cnt += 1
