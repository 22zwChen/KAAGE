...
def get_args():
    ...
    parser.add_argument("--name", default= 'gate2_' + time.strftime("[%Y-%m-%d__%H-%M-%S]", time.localtime()), help='Name of the experiment')
    parser.add_argument("--store_file", dest="store_file", default='results_gate2_' + time.strftime("[%Y-%m-%d__%H-%M-%S]", time.localtime()), help='file for storing the last results')
...
class Runner(object):
    ...
    def fit(self):
        ...
        read_path = os.path.join('./torch_saved/', 'credit_gate2_[2024-07-09__14-31-32]')
        if self.p.restore:
        ...
        if self.p.max_epochs == 0:
            epoch = 0
            self.logger.info('Evaluating on test data directly!')
            self.load_model(read_path)
        else:    
