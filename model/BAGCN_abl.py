...
class BAGCN(torch.nn.Module):
    ...
    def forward(self, ent_embed, rel_embed, edge_index, edge_type):
        ...
        num_edges = edge_index.size(1)   # E
        num_ent = ent_embed.size(0)  # N
        num_rel = rel_embed.size(0) 
        ...
        if agg == "gate2":   # gate2
            ...
            update_ent_embed = self.gate2_ent(self.residual_proj_ent(ent_embed),in_ent_embed)


            update_rel_embed = self.gate2_rel(in_rel_embed, self.residual_proj_rel(rel_embed))
